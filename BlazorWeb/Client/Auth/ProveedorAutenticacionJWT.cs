...
namespace BlazorWeb.Client.Auth
{
    public class ProveedorAutenticacionJWT : AuthenticationStateProvider, ILoginService
    {
        ...
        private async Task<string> RenovarToken(string token)
        {
            ...
            var nuevoTokenResponse = await repositorio.Get<UserToken>("api/usuarios/RenovarToken");
        ...
        private IEnumerable<Claim> ParseClaimsFromJwt(string jwt)
        {
            ...
            if (userID != null)
            {
                if (userID.ToString().Trim().StartsWith("["))
                {
                    ...
                    foreach (var parsed in parsedID)
                    {
                        claims.Add(new Claim(ClaimTypes.NameIdentifier, parsed));
                    }
                }
                else
                {
